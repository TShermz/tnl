...
export function getManagerNames (rosters, users, owner_id) {
  ...
  let managerRoster = rosters.filter((roster) => {return roster.roster_id === owner_id });

  //finds local manager's info based on sleeper id
  let localManagerInfo = managers.filter((manager) => {return managerRoster[0].owner_id === manager.sleeperID;});
  
  //finds the sleeper user info
  let user = users.filter((user) => {return managerRoster[0].owner_id === user.user_id;});

  //determines what the team name is
  let teamName = user[0].metadata.team_name? user[0].metadata.team_name: `Team ${user[0].display_name}`;
